...
def build_in_source_messages(taxon_names: list[str], source_text: str) -> list[dict]:
    ...
    prompt = (
        ...
        "text below. For each name, confirm whether it is literally mentioned in this "
        "source -- do not use outside biological knowledge; a real organism that is NOT "
        "present in this source text must still be marked not-in-source.\n\n"
...
def check_taxa_in_source(
    taxon_names: list[str], source_text: str, *, model: Model, stage: str = IN_SOURCE_STAGE
) -> set[str]:
    """Return the subset of `taxon_names` (exact original casing) the
    verifier confirms are literally present in `source_text`."""
    if not taxon_names:
        return set()
    response = model.complete(stage=stage, messages=build_in_source_messages(taxon_names, source_text))
...
def verify_signatures(
    ...
    *,
    ...
    grounded_names = check_taxa_in_source([taxon.taxon_name for taxon, _ in all_taxa], source_text, model=model)
